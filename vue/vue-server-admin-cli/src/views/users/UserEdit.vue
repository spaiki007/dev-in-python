    ...
    export default {
        ...
        mounted: function (){
            this.$nextTick(function(){
                ...
                axios.get(o.$store.state.domain + '/users/' +  o.$route.params.id).then(function(response){

                    for(let i=0; i<o.roles.length; i++){
                        if(o.roles[i].id == response.data.role){
                            o.role = o.roles[i].id;
                            o.activeSelectText = o.roles[i].name;
                            break;
                        }
                    }

                    o.login = response.data.login;
					o.name = response.data.name;
                    o.passw = response.data.passw;
        ...
        methods: {
            ...
            editObject: function(){
                ...
                axios.post(o.$store.state.domain + '/users/' + o.$route.params.id + '/update', data).then(function(response){
