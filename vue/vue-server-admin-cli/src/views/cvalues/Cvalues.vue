<template>
    <div>
        <div class="page-name">
            <h2>Значение характеристик</h2>
            <div class="add_object">
                <router-link :to="{name: 'cvalueAdd'}" class="btn btn-success" >Добавить</router-link>
            </div>
        </div>
        ...
        <form id="listObjects" class="form-delete" action="." method="post" accept-charset="utf-8" enctype="application/x-www-form-urlencoded">
            <div>
    			...
    			<div v-for="object in objects" :key="object.id" :class="['object']">
                    ...
                    <router-link :to="{name: 'cvalueEdit', params: { id: object.id }}" class="object_name" >{{ object.name }}</router-link>
    ...
    export default {
    	...
    	methods: {
    		...
    		deleteObjects: function(){
    			...
    			axios.post(o.$store.state.domain + '/cvalues/delete', formData).then(function(response){
					axios.get(o.$store.state.domain + '/cvalues/get').then(function(response){
	    				o.objects = JSON.parse(response.data.objects);
	    			}).catch(function (error){
	    				console.log(error);
    		...
    		chekedAll: function(){
    			for(let i=0; i<this.objects.length; i++) this.objects[i].checked = true;
    		},

    		unCheckedAll: function(){
    			for(let i=0; i<this.objects.length; i++) this.objects[i].checked = false;
    		},
