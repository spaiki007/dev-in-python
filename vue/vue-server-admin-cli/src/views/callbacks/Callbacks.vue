        ...
        <form id="listObjects" class="form-delete" action="." method="post" accept-charset="utf-8" enctype="application/x-www-form-urlencoded">
            <div>
    			...
    			<div v-for="object in objects" :key="object.id" :class="['object']">
                    ...
                    <router-link :to="{name: 'callbackEdit', params: { id: object.id }}" class="object_name" >{{ object.name }}</router-link>
    ...
    export default {
    	...
    	mounted: function (){
    		this.$nextTick(function(){
    			...
    			axios.get(o.$store.state.domain + '/callbacks/get').then(function(response){
    	...
    	methods: {
    		...
    		deleteObjects: function(){
    			...
    			axios.post(o.$store.state.domain + '/callbacks/delete', formData).then(function(response){
					axios.get(o.$store.state.domain + '/callbacks/get').then(function(response){
						o.objects = JSON.parse(response.data.objects);
					}).catch(function(error){
						console.log(error);
