    ...
    <div>
        <div class="page-name">
            ...
            <div class="add_object">
                <router-link :to="{name: 'productAdd'}" class="btn btn-success" >Добавить</router-link>
            </div>
        </div>
        ...
        <div class="form-delete">
			...
			<div v-for="object in objects" :key="object.id" :class="['object']">
                ...
                <router-link :to="{name: 'productEdit', params: { id: object.id }}" class="object_name" >{{ object.name }}</router-link>
    ...
    export default {
    	...
    	methods: {
    		...
    		deleteObjects: function(){
    			...
    			axios.post(o.$store.state.domain + '/products/delete', formData).then(function(response){
					axios.get(o.$store.state.domain + '/products/get').then(function(response){
	    				o.objects = JSON.parse(response.data.objects);
	    			}).catch(function (error){
	    				console.log(error);
