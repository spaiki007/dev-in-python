    ...
    <div>
        <div class="page-name">
            ...
            <div class="add_object">
                <router-link :to="{name: 'menuAdd'}" class="btn btn-success" >Добавить</router-link>
            </div>
        </div>
        ...
        <form id="listObjects" class="form-delete" action="." method="post" accept-charset="utf-8" enctype="application/x-www-form-urlencoded">
            <div>
    			...
    			<div v-for="object in objects" :key="object.id" :class="['object']">
                    ...
                    <router-link :to="{name: 'menuEdit', params: { id: object.id }}" class="object_name">{{ "—" . repeat(object.level) + " " + object.name }}</router-link>
    ...
    export default {
    	...
    	mounted: function (){
    		this.$nextTick(function(){
    			...
    			axios.get(o.$store.state.domain + '/menus/get').then(function(response){
    				o.objects = response.data.objects;
    	...
    	methods: {
    		...
    		deleteObjects: function(){
    			...
    			axios.post(o.$store.state.domain + '/menus/delete', formData).then(function(response){
					axios.get(o.$store.state.domain + '/menus/get').then(function(response){
	    				o.objects = response.data.objects;
	    			}).catch(function (error){
	    				console.log(error);
