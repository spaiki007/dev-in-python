    ...
    <div>
        <div class="page-name">
            ...
            <div class="add_object">
                <router-link :to="{name: 'optionAdd'}" class="btn btn-success" >Добавить</router-link>
            </div>
        </div>
        ...
        <form id="listObjects" class="form-delete" action="." method="post" accept-charset="utf-8" enctype="application/x-www-form-urlencoded">
            <div>
    			...
    			<div v-for="object in objects" :key="object.id" :class="['object']">
                    ...
                    <router-link :to="{name: 'optionEdit', params: { id: object.id }}" class="object_name" >{{ object.name }}</router-link>
    ...
    export default {
    	...
    	methods: {
    		...
    		deleteObjects: function(){
    			...
    			axios.post(o.$store.state.domain + '/options/delete', formData).then(function(response){
					axios.get(o.$store.state.domain + '/options/get').then(function(response){
	    				o.objects = JSON.parse(response.data.objects);
	    			}).catch(function (error){
	    				console.log(error);
