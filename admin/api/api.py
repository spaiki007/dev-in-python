#!/usr/bin/env python
# -*- coding: utf-8 -*-

from aiohttp import web
from authlib.jose import jwt
import requests, re, time

async def tinymce_upload(request):

	vimages = await request.multipart()
	
	while True:
	
		part = await vimages.next()
		if part is None:
			break
		
	return web.json_response({'': ''})


async def auth_jwt(request):
	
	secret = """-----BEGIN PRIVATE KEY-----

-----END PRIVATE KEY-----"""
	
	header	= {'alg': 'RS256'}
	payload	= {'sub': '123', 'name': 'spaiki007', 'exp': int(time.time()) + (60 * 10)}
	encoded = jwt.encode(header, payload, secret)
	
	return web.json_response({'token': encoded.decode("utf-8")})

def ya_translate(str_translate):
	
	replace = re.compile(r"[^\w]+", re.I)
	replace2= re.compile(r"^-|-$", re.I)
	
	data={
		'key': 'trnsl.1.1.20171124T160250Z.100ada6cec41aa5f.f15c5ea2aa9b2baef16bef4b0ca5cf724c2a2e2e',
		'text': str_translate,
		'lang': 'ru-en',
	}
	r = requests.post('https://translate.yandex.net/api/v1.5/tr.json/translate', data=data)
	
	result = replace.sub('-', r.json()['text'][0]).lower()
	result = replace2.sub('', result)
	
	return result