...
async def deleteQuickOrders(request):
	...
	async with request.app['db'].acquire() as conn:
		for _, v in data.items():
			await conn.execute(quickOrders.delete().where(quickOrders.c.id == v))
		return web.json_response({'delete': False})

	return web.json_response({'delete': False})

async def getQuickOrders(request):
	...
	async with request.app['db'].acquire() as conn:
		objects = await conn.execute(quickOrders.select())
...
async def getQuickOrder(request):
	...
	async with request.app['db'].acquire() as conn:
		object = await conn.execute(quickOrders.select().where(quickOrders.c.id == id))
...
async def updateQuickOrders(request):
	...
	async with request.app['db'].acquire() as conn:
		...
		await conn.execute(quickOrders.update().values(
			status = status,
		).where(quickOrders.c.id == id))
